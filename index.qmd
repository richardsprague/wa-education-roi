...
## Background: a constitutional promise, and what it cost to keep

Washington's constitution opens its education article with an unusually strong
commitment — 34 words that the Seattle Times' Dahlia Bazzaz calls the state's
"North Star" [@bazzaz2026paramount]:
...
They did. K–12 became the largest single item in the state budget. Washington
now has smaller class sizes, some of the highest teacher pay in the country,
and per-student spending just above the national average.

### What the money bought, and the question that follows
...
The narrowing matters, and it cuts against the headline. This analysis can say
...
## What NAEP is, and why the outcome is a scale score
...
### The problem: one state, no control group
...
### What "synthetic Washington" actually is
...
### How to tell whether a gap means anything: placebo tests
...
median untreated state diverges by **`r round(placebo_median, 2)` points**.
Washington is, to two decimal places, an ordinary state pretending nothing
happened. Its post/pre RMSPE ratio ranks `r inf$rank` of `r nrow(inf$table)`
...
### How large an effect could have hidden here?
...
What this analysis will not do is convert the second half into the first.
"Undetectable at this resolution" is not "zero," and a reader who wants
McCleary vindicated cannot find vindication here either: there is no positive
result to point at.
...
Those need a different design, and the outcome data for them is worse. It is
worth sitting with the possibility that the paramount-duty clause has done
exactly what it was read to require, funding K–12 to the point where marginal
